...
*/
PhysicLoader.BodyType = { Static: 0, Dynamic: 1, Kinematic: 2 };

/**
* Types of shape.
*/
PhysicLoader.ShapeType = { Circle: 0, Box: 1, Polygon: 2 };
...
*/
PhysicLoader.loadFromData = function( entity, data )
{
    // Create body.
    ...
    switch(data.type)
    {
        case PhysicLoader.BodyType.Dynamic:
            definition.set_type(b2_dynamicBody);
            break;
        case PhysicLoader.BodyType.Kinematic:
    ...
    var body = application.physic.world.CreateBody(definition);
    ...
    for( var i = 0; i < data.fixtures.length; i++ )
    {
        ...
        if( data.fixtures[i].shape.type == PhysicLoader.ShapeType.Circle )
        {
            var shape = new b2CircleShape();
            shape.set_m_radius((data.fixtures[i].shape.size * 0.5) - 0.01);
            fixture.set_shape(shape);
        }
        else if( data.fixtures[i].shape.type == PhysicLoader.ShapeType.Box ) 
        {
